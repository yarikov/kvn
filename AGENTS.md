...
| `update::key` | `src/app/update/key.rs` + `src/app/update/key/` | Keyboard input: `handle_key` routes by `Model.overlay` to `sources`, `confirm_delete`, `settings_menu`, `regions`, `dns`, `service_routing`, `theme`; `key/ipc.rs` (+ `ipc/{semantic,support}.rs`) handles `IpcCommand` for non-TUI clients |
...
| `migrations` | `src/migrations.rs`, `contrib/migrations/*.sh` | Ordered package migrations: root-owned script discovery, per-user/machine markers, runner lock, `profiles.json` backup and disposable candidate, validated single-write publish, end-of-run daemon restart handoff |
...
- **Legacy migration**: the old `settings.dns_strategy` field is still deserialized; the ordered profile-schema package migration promotes it into `dns.strategy` if the new value is still at its default. Ordinary config loading rejects legacy schemas and never runs `Config::migrate` implicitly. On save, `save_config_at` mirrors `dns.strategy` back into `dns_strategy` so older kvn builds keep loading the file.
...
| Migration candidate | `~/.config/kvn-tui/.profiles.json.migrating` |
